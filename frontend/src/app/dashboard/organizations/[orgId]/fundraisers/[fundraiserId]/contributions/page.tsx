...
export default function ContributionsPage({ params }: PageProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail || 'Failed to record contribution.';
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
        <h2 className="text-lg font-semibold text-gray-800">Record a Contribution</h2>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Transaction ID <span className="text-gray-400">(optional for manual)</span>
            </label>
            <input
              type="text"
              value={transactionId}
          ...
          >
            <div>
              ...
              <p className="text-xs text-gray-500">
                {c.payment_method} · {c.transaction_id || 'No transaction ID'} · {new Date(c.created_at).toLocaleDateString()}
              </p>
            </div>
